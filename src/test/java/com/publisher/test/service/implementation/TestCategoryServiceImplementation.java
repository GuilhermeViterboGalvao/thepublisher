...
public class TestCategoryServiceImplementation extends DefaultTest<Category> implements CategoryService {
	...
	@Override
	public ResultList<Category> search(String query, int page, int pageSize, Sort sort) {
		assertNotNull(query);
		assertTrue(!query.isEmpty());
		assertTrue(page >= 0);
		assertTrue(pageSize >= 0);
		assertNotNull(sort);
		ResultList<Category> search = categoryService.search(query, page, pageSize, sort);
		assertNotNull(search);
		return search;
	}
}