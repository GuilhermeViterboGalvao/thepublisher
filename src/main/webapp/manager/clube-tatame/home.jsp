<%@ page contentType="text/html" pageEncoding="UTF-8" %>
<div class="ym-form">
	<div class="ym-fbox-text">
		<h4 class="ym-fbox-heading">Itens</h4>
		<div class="ym-grid">
			<div class="ym-gbox">
				<a style="height: 30px; text-align: center; width: 95%;" class="ym-button" href="/manager/clube-tatame/gxml-edit?path=clube/home/destaque">Destaque</a>
			</div>
			<div class="ym-gbox">
				<a style="height: 30px; text-align: center; width: 95%;" class="ym-button" href="/manager/clube-tatame/gxml-edit?path=clube/home/chamadas">Chamadas</a>
			</div>
			<div class="ym-gbox">
				<a style="height: 30px; text-align: center; width: 95%;" class="ym-button" href="/manager/clube-tatame/gxml-edit?path=clube/home/note">Nota</a>
			</div>	
			<div class="ym-gbox">
				<a style="height: 30px; text-align: center; width: 95%;" class="ym-button" href="/manager/clube-tatame/gxml-edit?path=clube/home/indica">Quem indica</a>
			</div>			
		</div>
	</div>
</div>