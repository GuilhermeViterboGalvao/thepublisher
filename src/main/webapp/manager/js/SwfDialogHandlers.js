...
SWFHandlers.addImage = function(src, id, file, objHandler) {
	...
	inputId.type = "hidden";
	inputId.name = "swf_dialog_pictureId";
	inputId.value = id;
	...
	inputDesc.name = "swf_dialog_pictureDescription";
	inputDesc.type = "text";
	...
	inputTags.name = "swf_dialog_pictureTag";
	inputTags.type = "text";
...
SWFHandlers.uploadSuccess = function (file, serverData) {
	try {
		...
		if (id > 0) {
			...
			SWFHandlers.addImage("/upload/" + id + "_150x130.jpg?"+ (objHandler.alternativePath != undefined ? "path=" + objHandler.alternativePath : Math.random()*100), id, file, objHandler);
