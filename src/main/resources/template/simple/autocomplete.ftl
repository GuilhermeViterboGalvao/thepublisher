 ...
 */
-->
<span id="${parameters.id?default("")?html}_ACS">
	<a href="javascript:StrutsAC.search('${parameters.id?default("")?html}');">${parameters.display?default("-vazio-")?html}</a>
</span>
...
<script type="text/javascript">
	AutoComplete("${parameters.id?default("")?html}_ACSEngine",{
		url: "${parameters.url?default("")?html}",
		click: function(obj) {
			StrutsAC.update("${parameters.id?default("")?html}",obj, this<#if parameters.evaluator??>, ${parameters.evaluator}</#if>);
		},
		...
		<#if parameters.initial??>initial: ${parameters.initial},</#if>		
